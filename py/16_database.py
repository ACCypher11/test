...
class DatabaseManger:
    ...
    def get_user_posts(self, user_id):
        """Get posts by user"""
        try:
            # Convert srting user_id to ObjectId if it's a valid ObjectId
            if ObjectId.is_valid(user_id):
                user_object_id = ObjectId(user_id)
            else:
                user_object_id = user_id
    ...
    def update_user(self, user_id, updated_data):
        ...
        try:
            # Convert string user_id to ObjectId if it's valid
            if ObjectId.is_valid(user_id):
                user_object_id = ObjectId(user_id)
            else:
                user_object_id = user_id

            # Prevent updating the _id field accidentally
            if "_id" in updated_data:
                del updated_data["_id"]

            # Perform the update
            result = self.users_collection.update_one(
                {"_id": user_object_id},  # Find user by ID
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            # Build dictionary only with provided (non-empty) fields
            updated_data = {}
            if name:
            ...
            if age_input:
                try:
                    updated_data["age"] = int(age_input)
                except ValueError:
                    print("⚠ Invalid age input, must be a number.")
                    continue  # Skip and return to the main menu

            # If no fields were entered, skip the update
            if not updated_data:
            ...
            if db.update_user(user_id, updated_data):
                ...
                print("✗ Failed to update user (ID may not exist).")
            ...
            print("x Invalid choice. please enter 1-7.")
